...
module.exports.postCuerpoAcademicoDic = async (req, res) => {
  try {
    ...
    if (errorH) return res.json({ ok: false, errorH });
    await fsp
      .rename(
        `./Files/uploads/${dictamenDigitalizado}`,
        dir + dictamenDigitalizado
      )
      .catch(function errHandler(e) {
        errorH = e;
      });
    if (errorH) return res.json({ ok: false, errorH });
    ...
    if (!resp)
      return res.json({
        ok: false,
        message: 'No existe el documento en la bd'
      });
...
module.exports.putCuerpoAcademicoDic = async (req, res) => {
  try {
    ...
    if (!resp)
      return res.json({
        ok: false,
        message: 'Error al guardar el documento en la bd'
      });
